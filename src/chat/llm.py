...
def load_model_and_tokenizer(model_name: str, device: str, quantization: str) -> Tuple[AutoModelForCausalLM, AutoTokenizer, torch.device]:
...
class LocalLLMBridge:
    def __init__(self, model_name: str, listen_host: str, listen_port: int, next_ports: List[int], max_new_tokens: int = 128, device: str = "cpu", quantization: str = "none", temperature: float = 0.7, top_p: float = 0.9, max_history_turns: int = 12, history_char_budget: int = 4000, persist_history_items: int = 2000, system_prompt: str = "あなたは自然な日本語で簡潔に返答する会話AIです。") -> None:
        ...
        base_dir = script_dir.parent if script_dir.name == "llm" else script_dir
        ...
        print(f"[INFO] PID                   : {self.pid}")
        self.model, self.tokenizer, self.device_obj = load_model_and_tokenizer(model_name, device, quantization)
        self.generation_config = GenerationConfig(max_length=None, max_new_tokens=self.max_new_tokens, do_sample=(self.temperature > 0), temperature=self.temperature, top_p=self.top_p, pad_token_id=self.tokenizer.pad_token_id, eos_token_id=self.tokenizer.eos_token_id)
    ...
    def load_session_history(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.write_log({"event": "history_load_error", "session_id": session_id, "port": self.listen_port, "error": str(exc)})
        return []
    ...
    def save_session_history(self, session_id: str, history: List[Dict[str, Any]]) -> None:
        ...
        payload = {"session_id": session_id, "model": self.model_name, "updated_at": utc_timestamp(), "history": trimmed}
        tmp_path = session_path.with_suffix(".json.tmp")
        with open(tmp_path, "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)
        tmp_path.replace(session_path)

    def merge_histories(self, stored_history: List[Dict[str, Any]], incoming_history: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def build_prompt(self, input_text: str, history: List[Dict[str, Any]]) -> str:
        ...
        history_block = "\n".join(rendered_lines).strip() or "（履歴なし）"
    ...
    def generate_reply(self, input_text: str, history: List[Dict[str, Any]]) -> str:
        ...
        self.write_log({"event": "generate_metrics", "port": self.listen_port, "prompt_chars": len(prompt), "reply_chars": len(cleaned), "elapsed_sec": round(time.perf_counter() - started, 4)})
        return cleaned
    ...
    def handle_client(self, conn: socket.socket, addr: tuple[str, int]) -> None:
        ...
        try:
            ...
            self.write_log({"event": "receive", "session_id": session_id, "port": self.listen_port, "from_addr": f"{addr[0]}:{addr[1]}", "ttl": ttl, "input": input_text, "incoming_history_len": len(incoming_history), "stored_history_len": len(stored_history), "merged_history_len": len(history)})
            if ttl <= 0:
                self.write_log({"event": "end", "session_id": session_id, "port": self.listen_port, "ttl": ttl, "input": input_text, "history": history, "history_len": len(history)})
                conn.sendall(b"END")
                return
            ...
            self.write_log({"event": "reply", "session_id": session_id, "port": self.listen_port, "ttl": message["ttl"], "input": input_text, "reply": reply, "history": history, "history_len": len(history)})
            if message["ttl"] <= 0:
                self.write_log({"event": "final", "session_id": session_id, "port": self.listen_port, "ttl": 0, "input": input_text, "history": history, "history_len": len(history)})
                conn.sendall(b"FINAL")
                return
            next_port = self.send_next(message)
            self.write_log({"event": "forward", "session_id": session_id, "port": self.listen_port, "to_port": next_port, "ttl": message["ttl"], "input": input_text, "history": history, "history_len": len(history)})
            conn.sendall(b"OK")
        except Exception as exc:
            error_message = f"[ERROR] {exc}"
            print(error_message, file=sys.stderr)
            self.write_log({"event": "error", "session_id": message.get("session_id") if isinstance(message, dict) else None, "port": self.listen_port, "error": str(exc)})
            try:
    ...
    def serve_forever(self) -> None:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
            ...
            self.write_log({"event": "startup", "port": self.listen_port, "next_ports": self.next_ports, "model": self.model_name, "device": self.device, "quantization": self.quantization, "max_new_tokens": self.max_new_tokens, "max_history_turns": self.max_history_turns, "history_char_budget": self.history_char_budget, "persist_history_items": self.persist_history_items})
            while True:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--system-prompt", default="あなたは自然な日本語で簡潔に返答する会話AIです。")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    bridge = LocalLLMBridge(args.model, "127.0.0.1", args.listen_port, args.next_ports, args.max_new_tokens, args.device, args.quantization, args.temperature, args.top_p, args.max_history_turns, args.history_char_budget, args.persist_history_items, args.system_prompt)
