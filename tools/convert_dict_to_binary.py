...
def main() -> None:
    parser = argparse.ArgumentParser(description="階層型 dict.json を独自バイナリ形式へ変換します")
    ...
    if args.verify:
        ...
        if len(reloaded_entries) != len(entries):
            raise RuntimeError(f"Verification failed: input={len(entries)} output={len(reloaded_entries)}")
