...
class ImprovementCandidateWriter:
    """Append-only JSONL writer for teacher-derived improvement candidates."""

    def __init__(self, runtime_dir: str | Path = "runtime", *, rotate_latest: bool = True) -> None:
        self.runtime_dir = Path(runtime_dir)
        self.records_dir = self.runtime_dir / "review_candidates"
        self.records_dir.mkdir(parents=True, exist_ok=True)
        self.latest_path = self.records_dir / "latest.jsonl"
        if rotate_latest:
